  ...
  PaintUpDate() {
    ...
    for (let i = 0; i < this.rayMatrix.length; ++i) {
      ...
      for (let j = this.rayMatrix[i].length - 1; j >= 0; j--) {
        ...
        if (ray?.GObj?.texture) {
          // console.log(
          // (ray.GObj.rindringMetaData.mashQuantityRaysNum[ray.mashIndex]) )
          this.ctx.drawImage(
              ray.GObj.texture,
              Math.floor(ray.GObj.texture.width / ray.texturProcent * 100),
              0,
              Math.floor(
                  ray.GObj.texture.width /
                  (ray.GObj.rindringMetaData
                       .mashQuantityRaysNum[ray.mashIndex])),
